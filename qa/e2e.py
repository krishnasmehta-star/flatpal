...
async def main():
    async with async_playwright() as p:
        ...
        check("built page has 8 sections", (await page.locator('[data-testid^="emergent-section-"]').count()) == 8)
        txt = await page.locator("body").inner_text()
        ...
        check("built page has 6 proof bullets", (await page.locator('[data-testid="emergent-stat"]').count()) == 6)
        check("built page has 4 experience rows", (await page.locator('[data-testid^="emergent-exp-"]').count()) == 4)
        ...
        check("built page email CTA is mailto", (await page.locator('[data-testid="emergent-email-cta"][href^="mailto:krishna.s.mehta"]').count()) == 1)
        ...
        sheet = await mp.evaluate("document.querySelector('.sheet').getBoundingClientRect().width")
        check("mobile: sheet uses the width (small side padding)", sheet >= 340, str(sheet))
