...
async def main():
    async with async_playwright() as p:
        ...
        check("built page has 6 sections", (await page.locator('[data-testid^="emergent-section-"]').count()) == 6)
        txt = await page.locator("body").inner_text()
        check("no em dash on built page", "\u2014" not in txt)
        check("built page title", (await page.title()) == "Built on Emergent | FlatPal", await page.title())
        check("built page has 6 proof tiles", (await page.locator('[data-testid="emergent-stat"]').count()) == 6)
        check("built page has 7 JD rows", (await page.locator('[data-testid^="emergent-jd-"]').count()) == 7)
        check("built page has 3 plan windows and 3 gaps", (await page.locator('[data-testid^="emergent-plan-"]').count()) == 3 and (await page.locator('[data-testid^="emergent-gap-"]').count()) == 3)
        check("built page resume PDF link resolves", (await page.evaluate("fetch('/krishna-mehta-resume.pdf', {method:'HEAD'}).then(r => r.ok && (r.headers.get('content-type')||'').includes('pdf'))")))
        check("built page has 3 build rows", (await page.locator('[data-testid^="emergent-build-"]').count()) == 3)
        check("built page has 5 bugs", (await page.locator('[data-testid^="emergent-bug-"]').count()) == 5)
        check("built page has 3 experiments", (await page.locator('[data-testid^="emergent-experiment-"]').count()) == 3)
        check("built page CTAs point to /sample and /about", (await page.locator('[data-testid="emergent-sample-cta"][href="/sample"]').count()) == 1 and (await page.locator('[data-testid="emergent-about-cta"][href="/about"]').count()) == 1)
        check("built page shows email as text, no mailto anywhere", "krishna.s.mehta@gmail.com" in txt and (await page.locator('main a[href^="mailto:"]').count()) == 0)
        check("built page links the repo", (await page.locator('[data-testid="emergent-repo-link"][href="https://github.com/krishnasmehta-star/flatpal"]').count()) == 1)
        # Scroll through so every scroll-triggered reveal fires, then confirm nothing is left faded.
        await page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
        await page.wait_for_timeout(900)
        await page.evaluate("window.scrollTo(0, 0)")
        await page.wait_for_timeout(300)
        low = await page.evaluate(LOW_OPACITY_JS)
        check("built page fully visible after load", len(low) == 0, str(low[:3]))
        check("built page no placeholders left", "KRISHNA: confirm" not in txt)
        ...
        # Built on Emergent at a narrow phone width
        await mp.set_viewport_size({"width": 375, "height": 812})
        await mp.goto(f"{BASE}/built-on-emergent")
        await mp.wait_for_timeout(1800)
        sw = await mp.evaluate("document.documentElement.scrollWidth")
        ...
        check("mobile: stat tiles three per row", len(tiles) == 6 and abs(tiles[0] - tiles[2]) < 2 and abs(tiles[3] - tiles[5]) < 2 and tiles[3] - tiles[0] > 40, str(tiles))
        ...
        # Route change from the bottom of a long page must land at the top of the next page
        await mp.evaluate("window.scrollTo(0, document.body.scrollHeight)")
        await mp.wait_for_timeout(300)
        await mp.get_by_test_id("emergent-about-cta").click()
        await mp.wait_for_timeout(1200)
        check("mobile: story CTA lands at the top of About", "/about" in mp.url and (await mp.evaluate("window.scrollY")) < 5, str(await mp.evaluate("window.scrollY")))
